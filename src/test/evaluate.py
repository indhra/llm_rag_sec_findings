#!/usr/bin/env python3
"""
Full evaluation on the 13 test questions from the assignment.
"""

import os
import json
from datetime import datetime
from dotenv import load_dotenv

load_dotenv()

from src.pipeline import RAGPipeline

# The 13 test questions from the assignment
TEST_QUESTIONS = [
    # Q1-Q5: Apple questions
    {"question_id": 1, "question": "What was Apples total revenue for the fiscal year ended September 28, 2024?"},
    {"question_id": 2, "question": "How many shares of common stock were issued and outstanding as of October 18, 2024?"},
    {"question_id": 3, "question": "What is the total amount of term debt (current + non-current) reported by Apple as of September 28, 2024?"},
    {"question_id": 4, "question": "On what date was Apples 10-K report for 2024 signed and filed with the SEC?"},
    {"question_id": 5, "question": "Does Apple have any unresolved staff comments from the SEC as of this filing? How do you know?"},
    
    # Q6-Q10: Tesla questions
    {"question_id": 6, "question": "What was Teslas total revenue for the year ended December 31, 2023?"},
    {"question_id": 7, "question": "What percentage of Teslas total revenue in 2023 came from Automotive Sales (excluding Leasing)?"},
    {"question_id": 8, "question": "What is the primary reason Tesla states for being highly dependent on Elon Musk?"},
    {"question_id": 9, "question": "What types of vehicles does Tesla currently produce and deliver?"},
    {"question_id": 10, "question": "What is the purpose of Teslas 'lease pass-through fund arrangements'?"},
    
    # Q11-Q13: Out-of-scope questions (should be refused)
    {"question_id": 11, "question": "What is Teslas stock price forecast for 2025?"},
    {"question_id": 12, "question": "Who is the CFO of Apple as of 2025?"},
    {"question_id": 13, "question": "What color is Teslas headquarters painted?"},
]


def run_evaluation():
    """Run full evaluation and save results."""
    
    print("=" * 80)
    print("SEC 10-K RAG EVALUATION")
    print("=" * 80)
    
    # Initialize pipeline
    print("\nInitializing pipeline...")
    pipeline = RAGPipeline(
        embedding_model="bge-small",
        reranker_model="ms-marco-mini",
        llm_provider="groq",
        top_k_retrieval=15,  # Increased for better coverage
        top_k_rerank=7       # Increased to include more context
    )
    
    # Try to load existing index, otherwise build new one
    try:
        pipeline.load_index("outputs/index")
        print("Loaded existing index")
    except:
        print("Building new index...")
        pipeline.index_documents(data_dir="data/")
    
    # Run evaluation
    print("\n" + "=" * 80)
    print("RUNNING EVALUATION ON 13 QUESTIONS")
    print("=" * 80)
    
    results = []
    
    for item in TEST_QUESTIONS:
        qid = item["question_id"]
        question = item["question"]
        
        print(f"\n[Q{qid}] {question}")
        print("-" * 60)
        
        result = pipeline.answer_question(question, question_id=qid)
        
        print(f"Answer: {result['answer'][:300]}...")
        print(f"Sources: {result['sources']}")
        
        # Result already includes question_id, answer, sources in the right format
        results.append({
            "question_id": result["question_id"],
            "answer": result["answer"],
            "sources": result["sources"]
        })
    
    # Save results in the required format
    os.makedirs("outputs", exist_ok=True)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = f"outputs/evaluation_results_{timestamp}.json"
    
    with open(output_file, "w") as f:
        json.dump(results, f, indent=2)
    
    print("\n" + "=" * 80)
    print(f"EVALUATION COMPLETE")
    print(f"Results saved to: {output_file}")
    print("=" * 80)
    
    # Print the output format example
    print("\nOutput format:")
    print(json.dumps(results[:1], indent=2))
    
    # Print summary
    answerable = sum(1 for r in results if r["sources"])
    refused = sum(1 for r in results if not r["sources"])
    
    print(f"\nSummary:")
    print(f"  Questions answered: {answerable}")
    print(f"  Questions refused/not found: {refused}")
    
    return results


if __name__ == "__main__":
    run_evaluation()
