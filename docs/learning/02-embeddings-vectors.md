...
graph LR
    subgraph "Traditional: One-Hot Encoding"
        T1["'cat'"] --> V1["[0, 0, 1, 0, 0, ..., 0]<br/>50k dimensions<br/>Sparse"]
        T2["'dog'"] --> V2["[0, 1, 0, 0, 0, ..., 0]<br/>50k dimensions<br/>Sparse"]
    end
    
    subgraph "Modern: Dense Embeddings"
        T3["'cat'"] --> V3["[0.2, -0.5, 0.8, ...]<br/>384 dimensions<br/>Dense"]
        T4["'dog'"] --> V4["[0.3, -0.4, 0.7, ...]<br/>384 dimensions<br/>Dense"]
    end
...
graph LR
    subgraph "Embedding Dimensions"
        D1[Small: 128-384 dims] --> P1[✅ Fast search<br/>✅ Low memory<br/>⚠️ Less nuance]
        D2[Medium: 768-1024 dims] --> P2[⚖️ Balanced<br/>Standard choice]
        D3[Large: 1536+ dims] --> P3[✅ Rich semantics<br/>❌ Slow search<br/>❌ High memory]
    end
...
graph TD
    subgraph "Cosine Similarity Intuition"
        V1["Vector A: [1, 1]<br/>'cat kitten'"] 
        V2["Vector B: [1.5, 1.5]<br/>'feline animal'"]
        V3["Vector C: [1, -1]<br/>'car vehicle'"]
        
        V1 -->|Angle: 0°<br/>Similarity: 1.0| V2
        V1 -->|Angle: 90°<br/>Similarity: 0.0| V3
    end
...
graph TB
    subgraph "Bi-Encoder (SBERT, BGE)"
        ...
        P1[✅ Fast: Precompute embeddings<br/>✅ Scalable to millions<br/>⚠️ Less accurate for fine-grained]
    end
    ...
    subgraph "Cross-Encoder (Reranker)"
        ...
        P2[✅ Most accurate<br/>❌ Slow: Must compare all pairs<br/>Used for reranking top-K]
    end
...
graph LR
    subgraph "Pretrained Model"
        P[BGE pretrained<br/>on general web text] --> F[Your financial domain]
    end
    
    subgraph "Fine-Tuning Process"
        F --> D[Create pairs:<br/>Query → Relevant Doc]
        D --> T[Train with<br/>contrastive loss]
        T --> M[Fine-tuned model<br/>Better on 10-K filings]
    end
...
graph TD
    subgraph "Embedding Model Selection"
        ...
        D1 -->|<100ms| Small[Small model<br/>384 dims]
        D1 -->|<500ms| Medium[Medium model<br/>768 dims]
        D1 -->|Offline| Large[Large model<br/>1024+ dims]
...
graph TB
    subgraph "BGE-small-en-v1.5 Architecture"
        ...
        T --> E[BERT Encoder<br/>12 layers, 768 hidden]
        E --> P[Pooling<br/>CLS token or mean]
        P --> L[Linear Projection<br/>768 → 384 dims]
        L --> N[L2 Normalization]
        N --> O[Output Embedding<br/>384 dims, norm=1]
    end
